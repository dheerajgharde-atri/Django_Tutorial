...
class QuestionViewTests(TestCase):
    ...
    def test_past_question(self):
        new_question = create_question("Past question", -30)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerySetEqual(response.context['latest_question_list'], [new_question])

    def test_future_question(self):
        new_question = create_question("Future question", 30)
        response = self.client.get(reverse('polls:index'))
        self.assertContains(response, "No polls are available.")
        self.assertQuerySetEqual(response.context['latest_question_list'], [])
    ...
    def test_future_and_past_question(self):
        ...
        new_question = create_question("New Question", 30)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerySetEqual(response.context['latest_question_list'], [old_question])

    def test_two_old_questions(self):
        question1 = create_question("Question1", -30)
        question2 = create_question("Question2", -31)
        response = self.client.get(reverse('polls:index'))
...
class QuestionDetailViewTests(TestCase):
    ...
    def test_past_question(self):
        past_question = create_question("New Question", -30)
